...
// take it over from someone who's leaving, or step back — no Commish needed.
// "Group" = your flight if you're in one, else the whole round. See
// claim_scorer / release_scorer (migration 00030).
...
export function ScorerControl({ roundId, scorerId, scorerName, isMe }: Props) {
  ...
  // Someone else is scoring → they own the official card; offer to take over.
  if (scorerId) {
    return (
      <div className="flex items-center justify-between gap-3 rounded-lg border border-surface-600 bg-surface-700/60 px-4 py-2.5">
        <span className="text-sm text-surface-200">
          <span className="font-medium text-surface-100">{scorerName ?? 'Someone'}</span> is
          keeping score. You can still track your own card.
        </span>
        <button
